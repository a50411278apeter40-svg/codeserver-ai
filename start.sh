  ...
  if [ -n "${GITHUB_TOKEN:-}" ] && [ -n "${GITHUB_SANDBOX_REPO:-}" ]; then
    ...
    if [ -n "$CS_NAME" ]; then
      ...
      if [ -s /root/.ssh/codespace_config ]; then
        ...
        # `gh codespace ssh --config` emits a ProxyCommand that calls
        # `gh cs ssh -c <name> --stdio -- ...` directly, with no check that
        # the codespace is actually running right now. Codespaces auto-stop
        # after ${idle_timeout_minutes} of inactivity (independent of this
        # Render container, which just keeps running) — so any ssh attempt
        # AFTER that point (new terminal tab, reconnect, the rsync loop's
        # next tick, tomorrow) hit a stopped codespace and failed outright
        # (exit 255). Rewrite the ProxyCommand to go through ssh_proxy.js,
        # which wakes the codespace (waits for Available) before every
        # single connection attempt, not just once at container boot.
        sed -i "s#^\(\s*ProxyCommand\).*#\1 node /app/backend/ssh_proxy.js ${CS_NAME}#" /root/.ssh/codespace_config
        ...
        echo "[start.sh] SSH config ready — host alias: ${CS_HOST} (ProxyCommand wraps ssh_proxy.js so it wakes the codespace on every connect, not just at boot)."
